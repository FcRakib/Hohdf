...
global_premium_accounts = []  # Global storage for all premium accounts
user_data = {}
public_data = {}
checking_status = {}
in_progress = {}
...
def send_new_user_notification(user):
    bot.send_message(ADMIN_CHAT_ID, f"🆕 *New User Detected!*\n\n*Name*: {user['name']}\n*Username*: {user['username']}\n*UserID*: {user['user_id']}\n*Total Users*: {len(new_users)}")
...
def handle_combo_upload(message):
    ...
    if not is_user_member(chat_id):
        bot.reply_to(message, f"""❌ *You must join our channel to use this bot.*

👉 [Join Channel](https://t.me/{CHANNEL_USERNAME})""", parse_mode="Markdown", disable_web_page_preview=True)
        return
    ...
    if not is_premium_user(chat_id):
        bot.send_message(chat_id, f"""✅ *File received. The file contains {combo_count} combos.*

⚠️ *As a free user, only the first 1000 combos will be checked.*""", parse_mode="Markdown")
    else:
        bot.send_message(chat_id, f"✅ *File received. The file contains {combo_count} combos.*", parse_mode="Markdown")
...
"""
        ...
        bot.send_message(chat_id, popup_text, parse_mode="Markdown", reply_markup=markup)
        return
    ...
    if is_premium_user(chat_id):
        bot.send_message(chat_id, f"✅ *{mode.capitalize()} Mode Activated!*", parse_mode="Markdown")
        threading.Thread(target=start_combo_check, args=(chat_id, combo_file)).start()
    else:
        queue_position = get_queue_position(chat_id)
...
"""
    ...
    bot.edit_message_text(progress_text, chat_id, inline_message_id, reply_markup=markup, parse_mode="Markdown")
...
def handle_stop(call):
    ...
    bot.send_message(chat_id, "⛔ *Process Stopping... Please wait for the summary.*", parse_mode="Markdown")
...
def start_combo_check(chat_id, combo_file):
    ...
    inline_message = bot.send_message(chat_id, "*Starting to check combos...*", parse_mode="Markdown")
        ...
        try:
            ...
            if result == 'premium':
                ...
                if user_mode[chat_id] == "spam":
                    bot.send_message(chat_id, format_hit_spam(email, password), parse_mode="Markdown")
            elif result == 'error':
                error += 1
            else:
                bad += 1
...
⚠️ *As a free user, only the first 1000 combos will be checked.*""", parse_mode="Markdown")
        ...
        bot.send_message(chat_id, "❌ *Please reply to a combo file with* /chkf *to start processing.*", parse_mode="Markdown")
...
👉 [Join Channel](https://t.me/{CHANNEL_USERNAME})""", parse_mode="Markdown", disable_web_page_preview=True)
    ...
    if ':' in text:
        ...
        bot.send_message(chat_id, f"*Result*: {result.capitalize()}", parse_mode="Markdown")
    else:
        bot.send_message(chat_id, "❌ *Invalid format. Use* `/chk email:password`", parse_mode="Markdown")
...
👉 [Join Channel](https://t.me/{CHANNEL_USERNAME})""", parse_mode="Markdown", disable_web_page_preview=True)
    ...
    if chat_id in user_data and user_data[chat_id]:
        ...
        with open(f"premium_hits_{chat_id}.txt", 'rb') as file:
            bot.send_document(chat_id, file, caption="📄 Here are your premium and Valid accounts !", parse_mode="Markdown")
...
💡 *Check some combos to find premium accounts!*""",
            parse_mode="Markdown"
        )
...
👉 [Join Channel](https://t.me/{CHANNEL_USERNAME})""", parse_mode="Markdown", disable_web_page_preview=True)
    ...
    if chat_id in user_data and user_data[chat_id]:
        ...
        with open(f"combo_hits_{chat_id}.txt", 'rb') as file:
            bot.send_document(chat_id, file, caption="📄 *Here are your combo hits!*", parse_mode="Markdown")
    else:
        bot.reply_to(
            message,
            """❌ *No combo hits found.*

💡 *Send some combos to store them!*""",
            parse_mode="Markdown"
        )
...
👉 [Join Channel](https://t.me/{CHANNEL_USERNAME})""", parse_mode="Markdown", disable_web_page_preview=True)
    ...
    if chat_id in user_cooldowns:
        if current_time < user_cooldowns[chat_id]:
            ...
            bot.reply_to(message, f"⏳ *You can use this command again in {remaining_time.seconds // 3600} hours and {(remaining_time.seconds % 3600) // 60} minutes.*", parse_mode="Markdown")
            return
    ...
    if global_premium_accounts:
        ...
        bot.reply_to(message, "❌ *No premium accounts available at the moment. Please try again later.*", parse_mode="Markdown")
...
💡 *Start sending combos  to contribute!*""",
            parse_mode="Markdown"
        )
        return
...
✅ *Exclusive Features*: AccesSPAM M upcoming premium-onlyfeatures
...
"""
    ...
    bot.send_message(chat_id, premium_benefits, parse_mode="Markdown", reply_markup=markup)
...
def add_premium_user(message):
    ...
    try:
        ...
        bot.reply_to(message, f"✅ *User {user_id} has been added as a premium user.*", parse_mode="Markdown")
    except (IndexError, ValueError):
        bot.reply_to(message, "❌ *Invalid command format. Use* `/add user_id`", parse_mode="Markdown")
...
def remove_premium_user(message):
    ...
    try:
        ...
        if user_id in premium_users:
            ...
            bot.reply_to(message, f"✅ *User {user_id} has been removed from premium users.*", parse_mode="Markdown")
        else:
            bot.reply_to(message, f"❌ *User {user_id} is not a premium user.*", parse_mode="Markdown")
    except (IndexError, ValueError):
        bot.reply_to(message, "❌ *Invalid command format. Use* `/rmv user_id`", parse_mode="Markdown")

@bot.message_handler(commands=['leavequeue'])
def handle_leave_queue(message):
    chat_id = message.chat.id
    if leave_queue(chat_id):
        bot.reply_to(message, "✅ *You have successfully left the queue.*", parse_mode="Markdown")
    else:
        bot.reply_to(message, "❌ *You are not currently in the queue.*", parse_mode="Markdown")
...
def handle_leave_queue_button(call):
    ...
    if leave_queue(chat_id):
        ...
        bot.edit_message_text("✅ *You have successfully left the queue.*", chat_id, call.message.message_id, parse_mode="Markdown")
    else:
        bot.answer_callback_query(call.id, "You are not currently in the queue.")
        bot.edit_message_text("❌ *You are not currently in the queue.*", chat_id, call.message.message_id, parse_mode="Markdown")
